...
const CreateKpPrev = (props) => {
  ...
  const {isLoading: isLoadingHtsCode} = useQuery([getHtsCodeKey, patientObj?.id], () => fetchHtsCode(patientObj?.id), {
    onSuccess: (data) => {
      setHtsCodeVal({
  ...
  const {isLoading: isLoadingPrevCode} =  useQuery([getPrepCodeKey, patientObj?.id], () => fetchPrepCode(patientObj?.id), {
    onSuccess: (data) => {
      setPrepCodeVal({
  ...
  const [facilityRefferedToo] = useState({
